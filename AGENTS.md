...
tools/           ← Standalone CLI tools
  Codex-memory-sync/   ← Cross-machine memory sync via git
scripts/         ← Dev tooling
  check-skill-sync.sh
```
...
If you clone this repo and want edits to sync automatically with your Codex config, use hard links instead of copying:

```bash
# macOS / Linux
ln skills/*.md ~/.Codex/commands/
ln agents/*.md ~/.Codex/agents/
```

```powershell
# Windows
Get-ChildItem skills\*.md | ForEach-Object { New-Item -ItemType HardLink -Path "~\.Codex\commands\$($_.Name)" -Target $_.FullName }
Get-ChildItem agents\*.md | ForEach-Object { New-Item -ItemType HardLink -Path "~\.Codex\agents\$($_.Name)" -Target $_.FullName }
```
...
**Sync check:** Add a `SessionStart` hook to `~/.Codex/settings.json` to detect drift at the start of every Codex session:
