...
function Export-CodexLaunchWrappers {
    ...
    $toolScriptPath = Join-Path $script:ToolRoot "codex-setup-sync.ps1"
    ...
    $codexCommand = if ($Config.launch.codex_cli) { $Config.launch.codex_cli } else { "codex" }
    $codexApp = if ($Config.launch.codex_app) { $Config.launch.codex_app } else { "" }
...
`$env:CODEX_SETUP_SYNC_CONFIG = '$configPath'
& '$toolScriptPath' sync
...
`$appCommand = '$codexApp'
if (-not `$appCommand) {
    Write-Host 'Set launch.codex_app in your codex-setup-sync config before using this wrapper.' -ForegroundColor Yellow
    exit 1
}
